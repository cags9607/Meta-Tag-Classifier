...
def select_meta_with_nav_and_quality(df: pd.DataFrame,
    ...
    df = df.copy()
    titles = df.get("title_meta_domain_removed", pd.Series([""] * len(df), index = df.index)).fillna("")
    descs  = df.get("description_meta_domain_removed", pd.Series([""] * len(df), index = df.index)).fillna("")
    jaccs  = df.get("jaccard_similarity", pd.Series([None] * len(df), index = df.index))

    title_meta_names = df.get("title_meta_name", pd.Series([pd.NA] * len(df), index = df.index))
    description_meta_names = df.get("description_meta_name", pd.Series([pd.NA] * len(df), index = df.index))

    rows = []
    for t, d, j, title_meta_name, description_meta_name in zip(
        titles,
        descs,
        jaccs,
        title_meta_names,
...
def clean_metas(df: pd.DataFrame,
    ...
    if "title_meta_name" not in pivot_df.columns:
        pivot_df["title_meta_name"] = [
            ...
            for t, ot, tt in zip(
                pivot_df.get("title", ""),
                pivot_df.get("og:title", ""),
                pivot_df.get("twitter:title", ""),
            )
        ]
    ...
    if "description_meta_name" not in pivot_df.columns:
        pivot_df["description_meta_name"] = [
            ...
            for d, od, td, md in zip(
                pivot_df.get("description", ""),
                pivot_df.get("og:description", ""),
                pivot_df.get("twitter:description", ""),
                pivot_df.get("meta_description", ""),
            )
        ]
