...
def collapse_ngram_runs(s: str, n: int = 2) -> str:
    ...
    while i < L:
        ...
        j = i + n
        while j + n <= L and toks[i:i + n] == toks[j:j + n]:
            j += n
        i = j
    return " ".join(out)
...
def check_domain_in_text(domain: str, text: str) -> int:
    if not isinstance(domain, str) or not isinstance(text, str):
        return 0
    parts = domain.split(".")
    domain_word = parts[-2] if len(parts) > 1 else parts[0]
...
def scan_nav_noise(text: str, nav_phrases: Iterable[str] = DEFAULT_NAV_PHRASES, sep_bonus: bool = True, tail_bonus: bool = True):
    s = nfkc_casefold(text or "")
    n = max(1, len(s))
    hits = 0
    unique = set()
    score = 0.0
    by_phrase = defaultdict(int)

    def local_bonus(pos):
        bonus = 1.0
        if sep_bonus:
            window = s[max(0, pos - 5):min(n, pos + 15)]
            if any(sep.strip() in window for sep in SEPARATORS):
                bonus *= 1.3
        if tail_bonus and pos >= int(0.7 * n):
            bonus *= 1.5
        return bonus
    ...
    return {
        "hits_total": hits,
        "hits_unique": len(unique),
        "by_phrase": dict(by_phrase),
        "density": density,
        "score": float(score),
        "norm_len": n,
    }
...
def repetition_diversity(tokens):
    T = len(tokens)
    if T == 0:
        return {"ttr": 0.0, "max_token_share": 1.0, "rep3_share": 0.0}
...
def structural_quality(text: str, field: str, jaccard_similarity_val: Optional[float] = None):
    ...
    return {
        ...
        "charlen": n,
        "ttr": div["ttr"],
        "max_token_share": div["max_token_share"],
        "rep3_share": div["rep3_share"],
        "pen_trunc": pen_trunc,
        "pen_scar": pen_scar,
        "pen_dup": pen_dup,
    }
...
def select_meta_with_nav_and_quality(
    df: pd.DataFrame,
    ...
    df = df.copy()
    titles = _col_or_empty(df, "title_meta_domain_removed")
    descs = _col_or_empty(df, "description_meta_domain_removed")
    jaccs = df["jaccard_similarity"] if "jaccard_similarity" in df.columns else pd.Series([None] * len(df), index = df.index)
    title_meta_names = df["title_meta_name"] if "title_meta_name" in df.columns else pd.Series([pd.NA] * len(df), index = df.index)
    description_meta_names = df["description_meta_name"] if "description_meta_name" in df.columns else pd.Series([pd.NA] * len(df), index = df.index)

    rows = []
    for t, d, j, title_meta_name, description_meta_name in zip(titles, descs, jaccs, title_meta_names, description_meta_names):
        ...
        if src == "title":
            selected_name = title_meta_name
        elif src == "desc":
            selected_name = description_meta_name
        else:
            selected_name = pd.NA

        if isinstance(selected_name, str) and not selected_name.strip():
            selected_name = pd.NA

        rows.append({
            "title_nav_hits": sc_t["hits_total"],
            "desc_nav_hits": sc_d["hits_total"],
            "title_nav_score": sc_t["score"],
            "desc_nav_score": sc_d["score"],
            "is_noisy_title": is_noisy_t,
            "is_noisy_desc": is_noisy_d,
            "title_q": q_t["q"],
            "desc_q": q_d["q"],
            "title_charlen": q_t["charlen"],
            "desc_charlen": q_d["charlen"],
            "title_pen_trunc": q_t["pen_trunc"],
            "desc_pen_trunc": q_d["pen_trunc"],
            "title_pen_scar": q_t["pen_scar"],
            "desc_pen_scar": q_d["pen_scar"],
            "title_pen_dup": q_t["pen_dup"],
            "desc_pen_dup": q_d["pen_dup"],
            "selected_src": src,
...
def _first_non_empty_name(pairs):
...
def clean_metas(
    ...
    title_col = _col_or_empty(pivot_df, "title")
    og_title_col = _col_or_empty(pivot_df, "og:title")
    twitter_title_col = _col_or_empty(pivot_df, "twitter:title")

    desc_col = _col_or_empty(pivot_df, "description")
    og_desc_col = _col_or_empty(pivot_df, "og:description")
    twitter_desc_col = _col_or_empty(pivot_df, "twitter:description")
    meta_desc_col = _col_or_empty(pivot_df, "meta_description")

    if "title_meta" not in pivot_df.columns:
        pivot_df["title_meta"] = [
            _first_non_empty(t, ot, tt)
            for t, ot, tt in zip(title_col, og_title_col, twitter_title_col)
        ]

    if "description_meta" not in pivot_df.columns:
        pivot_df["description_meta"] = [
            _first_non_empty(d, od, td, md)
            for d, od, td, md in zip(desc_col, og_desc_col, twitter_desc_col, meta_desc_col)
        ]

    if "title_meta_name" not in pivot_df.columns:
        pivot_df["title_meta_name"] = [
            _first_non_empty_name([
                ("title", t),
                ("og:title", ot),
                ("twitter:title", tt),
            ])
    ...
    if "description_meta_name" not in pivot_df.columns:
        pivot_df["description_meta_name"] = [
            _first_non_empty_name([
                ("description", d),
                ("og:description", od),
    ...
    if "target_domain" not in pivot_df.columns:
        pivot_df["target_domain"] = ""
    ...
    pivot_df["title_has_domain_name"] = pivot_df.apply(
        lambda r: check_domain_in_text(r["target_domain"], r["title_meta"]),
        axis = 1,
    )
    pivot_df["desc_has_domain_name"] = pivot_df.apply(
        lambda r: check_domain_in_text(r["target_domain"], r["description_meta"]),
        axis = 1,
    )
    ...
    pivot_df["title_meta_domain_removed"] = pivot_df.apply(
        lambda r: remove_domain_from_text(r["target_domain"], r["title_meta_cleaned"], r["title_has_domain_name"]),
        axis = 1,
    )
    pivot_df["description_meta_domain_removed"] = pivot_df.apply(
        lambda r: remove_domain_from_text(r["target_domain"], r["description_meta_cleaned"], r["desc_has_domain_name"]),
        axis = 1,
    )
    ...
    out = select_meta_with_nav_and_quality(
        pivot_df,
        nav_phrases = DEFAULT_NAV_PHRASES,
        nav_thr = nav_thr,
