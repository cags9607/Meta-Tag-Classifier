...
def pop(batch_size: int = 1) -> list:
    ...
    try:
        logger.debug(f"Requesting {batch_size} jobs from queue (key={QUEUE_KEY})")
        ...
        logger.error(f"Failed to fetch jobs from queue: {str(e)}")
        raise
...
def push(processed_jobs: list):
    ...
    try:
        logger.debug(f"Pushing {len(processed_jobs)} processed job bundles to queue (key={QUEUE_KEY})")
        response = requests.post(url = QUEUE_URL, json = data, headers = headers)
        response.raise_for_status()
        logger.debug("Successfully pushed processed jobs to queue")
    except Exception as e:
        logger.error(f"Failed to push processed jobs to queue: {str(e)}")
        raise
